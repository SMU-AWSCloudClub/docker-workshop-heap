...
docker compose -f docker-compose.cloud-ssl.yaml run --rm certbot certonly --webroot --webroot-path=/var/www/certbot $PUBLIC_IP
...
docker compose -f docker-compose.cloud-ssl.yaml up --build